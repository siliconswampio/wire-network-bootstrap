...
wire-setup() {
  ...
  while true; do
      ...
      initial_hash=$(cleos get table eosio eosio abihash | jq -r '.rows[0].hash')
      echo "Initial hash: $initial_hash"

      # Loop until the contract is successfully deployed
      while true; do
          echo "Deploying eosio.system contract...";
          if cleos set contract eosio /opt/eosio.contracts/build/contracts/eosio.system -x 3600; then
      ...
      new_hash=$(cleos get table eosio eosio abihash | jq -r '.rows[0].hash')
  ...
  # buyram for eosio
  cleos push transaction '{"delay_sec":0,"actions":[{"account":"eosio","name":"buyram","data":{"payer":"eosio","receiver":"eosio","quant":"5.0000 EOS"},"authorization":[{"actor":"eosio","permission":"active"}]}]}' ;

  # delegatebw for eosio
  cleos push transaction '{"delay_sec":0,"actions":[{"account":"eosio","name":"delegatebw","data":{"from":"eosio","receiver":"eosio","stake_net_quantity":"10.0000 EOS","stake_cpu_quantity":"5.0000 EOS","transfer":false},"authorization":[{"actor":"eosio","permission":"active"}]}]}' ;

  # Activate Sig EM Key Type "feature_digest": "03d8a26c72bd89ce9c2217c4d6eebe893100f9bc6b8baf14b577137785894e10"
  cleos push action eosio activate '["03d8a26c72bd89ce9c2217c4d6eebe893100f9bc6b8baf14b577137785894e10"]' -p eosio  
}
